...
/*------------------------------------------*/
/*--------------设置系统关闭时间--------------*/
char cmd_osoff_delaytime_set[]={"setosofftime=%d/%d/%d/%d.%d.%d//"};//年/月/日/时.分.秒
/*------------------------------------------*/
/*-----------------自定义指令区--------------*/
...
{
    struct memblock *new=(void*)((INT8U*)fitting_slot+size+sizeof(struct memblock));  //重新定位剩余空闲的地址
...
void OS_Information_Process(void)
{
  ...
  OSCMDprintf("  运行模式 ：%s                     运行时间：%d年%d月%d日 %d时%d分%d秒 %dms\r\n",
                                             ...
                                             0,
	                                           ...
	                                           0);
#endif	
	OSCMDprintf("  时钟节拍 ：%dms                            系统关闭倒计时：%d年%d月%d日 %d时%d分%d秒\r\n",\
	...
	for(TaskCount = 0; TaskCount < OS_MAX_TASKS; TaskCount++) 
	{
		if(OSTCBTbl[TaskCount].StkPtr!=(STK32U*)0)
		{	
			OSCMDprintf(" %04.1f%%    %03d       %-3d     %-3d     %04.1f%%     %04d     %05d     %d:%d    %s \r\n",\
...
{	
	INT32U i;
	OSprintf("-@FSC_STOS_V4.9.5 Inside\r\n");
...
INT8U OSMutexPend(INT8U pendtype,INT16U MNum,INT32U timeout_ms) //等待互斥量 带超时时间  ，返回OS_TRUE：等待未超时  OS_FALSE：等待超时
{
	INT8U result;
	OSSchedLock();
	...
	OSTCBCur->TaskFlagBit|=(1<<_BIT_Mutex);//置位等待mutex标志
	OSSchedUnlock();
	switch(pendtype)
	{
		case OSMutex_BPN:
		{
			OS_delayMs(timeout_ms);
			OSTCBCur->TaskFlagBit&=~(1<<_BIT_Mutex);//清除等待mutex标志
			if(OSTCBCur->TaskFlagBit&(1<<_BIT_TimeOut)) //超时检测
			{	
				OSTCBCur->TaskFlagBit&=~(1<<_BIT_TimeOut);//清除超时标志位
				result = OS_FALSE;
