...
void os_timer_add(os_timer* timer)
{
	  ...
	  os_timer *os_timer_temp=os_timer_list_head->next;
		os_timer *os_timer_insert;
		if(timer->type==timer_type__thread)
		{
			os_timer_insert=os_thread_timer_free_get();
			*os_timer_insert=*timer;
		}
		else
		{
			os_timer_insert=timer;
		}
		os_timer_insert->bool_alarm_end=os_false;
		if(os_timer_temp==os_timer_list_rear)
    {
			os_timer_list_insert(os_timer_insert,0);		
	  }
	  else
    {		
			i=0;
			time_ticks=os_timer_insert->value;
			os_timer_insert->value=0;
			while(os_timer_temp!=os_timer_list_rear)
			{	
				time_sum+=os_timer_temp->value;		
				if(time_sum>time_ticks)
				{
					os_timer_insert->value=time_ticks-time_sum_last+1;
					os_timer_list_insert(os_timer_insert,i);
					os_timer_insert->next->value=time_sum-time_ticks+1;//+1是为了保证最小值为1(==0设计为未使用,==1为最小使用值)
					break;
				}
			...
			{	
				...
				{	
					i++;
					if(os_timer_temp->value>0) 
					{
						j=i;
					}	
					os_timer_temp=os_timer_temp->next;
				}
				os_timer_insert->value=time_ticks-time_sum+1;
