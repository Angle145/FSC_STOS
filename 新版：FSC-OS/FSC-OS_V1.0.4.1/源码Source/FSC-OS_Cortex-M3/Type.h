...
typedef os_u32         stk32;          
typedef stk32          os_stk32;  

#define os_mem_stack __align(8) os_mem         
/*-----------------------------------*/

//错误代码
typedef enum{
  error_code_type__01=1,
	error_code_type__02,
}os_error_code_type;

//APP状态
typedef enum{
  os_app_state_creating=1,                         //创建态
...
//Thread状态
typedef enum{
  os_thread_state_creating=1,                       //创建态
...
//优先及链表状态
typedef enum{
  os_thread_prio_state_readying=1,                       //就绪态
...
typedef enum 
{
  timer_mode_type__once=1,
	timer_mode_type__cycle,
}os_timer_mode_type;
...
struct os_timer_struct
{
	os_u32 type;           //定时器类型
	os_u32 para[8];        //参数
	os_u32 value;  				//定时器累计变量
	os_u32 reload_value;
	os_u32 thread_prio;
	os_u32 thread_global_prio;
	os_u32 id;             //定时器列表节点编号
	os_timer_mode_type timer_mode;
	os_bool bool_sleep;
	os_bool bool_alarm_end; //定时到时一次(自动重装模式下进入下次定进会被清除)
	os_bool bool_alarm_ring;//定时到时(自动重装模式下进入下次定进不会被清除)
	os_bool bool_delete;
