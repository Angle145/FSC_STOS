;说明：M0内核仅支持直接访问CPU寄存器R0-R7，R8-R15必须通过R0-R7作为中间媒介来操作,该方法适用所有M内核。
;M3及M3以上支持直接访问R0-R15，该方法仅适用M3及以上内核。

;阅读以下代码时，需要配合Cortex-M0(M3)权威指南书，了解寄存器组结构和作用，掌握Cortex-M汇编语言基础。
Cortex_M0     EQU   0              ;can be used  //等价C语言：#define Cortex_M0 0
Cortex_M3     EQU   1              ;can be used  //           #define Cortex_M3 1	
Cortex_M4     EQU   2              ;can be used  //           #define Cortex_M3 2
	
;*---------------------内核选择(Kernel Set)-------------------*	
;Kernel    EQU Cortex_M0	            ;for stm32f0xx                   //#define Kernel Cortex_M0
;Kernel    EQU Cortex_M3	            ;for stm32f1xx                   //#define Kernel Cortex_M3
Kernel    EQU Cortex_M4	            ;for stm32f4xx                   //#define Kernel Cortex_M3
;*------------------------------------------------------------*

	IMPORT    os_tcb_cur         ;//外部变量声明:            extern oc_tcb *os_acb_cur;
    IMPORT    os_tcb_rdy        ;//                         extern oc_tcb *os_acb_next;
...
;------------------------Cortex_M0 Code	Start---------------------; //M0内核有关代码开始
    ...
    ;//不会影响到栈顶到栈顶-16这段内栈的值。特别说明下，并非所有代码运行时都用到R4-R12,经常用到的只有R0-R3，所以
    ;//有时R4-R12是用不到的，虽然上下文切换时也保存和恢复了它们，但是它们的值可能一直为0，这样在计算任务栈使用率
    ;//时就会显示这段内存是空闲的，所以有时任务栈可能只用到几个必要的寄存器xPSR,PC,LR,R12,R3-R0。	
	;//M0中，先保存R4-R7是因为R8-R11要通过R4-R7为媒介才能操作，为了不破环R4-R7的值，所以先保存R4-R7的值,当然，也可以
...
;+++++++++++++++++++Save Context Code(PageDown)+++++++++++++++++++;	//任务环境保存结束	
    ...
    LDR     R1, =os_tcb_cur      ;//读取OSTCBCur指针值到R1，不用R0是因此时R0上有保存R寄存器后的内存地址。
...
OSFirstEnter                   ;//第一次进入系统标号(第一次进入系统，不需要保存R寄存器值，因为此时的值是无用的)
    LDR     R0, =os_tcb_cur      ;//读取OSTCBCur指针值到R0    
    LDR     R1, =os_tcb_rdy     ;//读取OSTCBNext指针值到R1  
