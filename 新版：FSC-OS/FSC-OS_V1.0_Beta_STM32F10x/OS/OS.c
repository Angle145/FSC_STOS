...
void os_timer_counter_process(void) //系统定时器程序
{
	...
	if(os_timer_list_front->next->clock_counter>1) os_timer_list_front->next->clock_counter--;//最小值设计为1
	else 
	{
      ...
      switch(os_timer_list_front->next->type)		
			{	
				case timer_type__app: 
				{
					os_timer_list_strutc_erase(os_timer_list_front->next);//从定时器定时列表中删除	    
				}
        case timer_type__thread:
				{					
					os_timer_temp=os_timer_list_front->next;//备份timer_list_front	
					os_timer_list_strutc_erase(os_timer_list_front->next);//从定时器定时列表中删除	
				...
				break;
				default: os_timer_list_strutc_erase(os_timer_list_front->next);//从定时器定时列表中删除	
				break;
			}				
...
void os_tcb_ptr_init(void)
{
	...
	//开机先运行空闲线程
    //os_tcb_cur = os_tcb_prio_sort_table_list_front->list_same_prio_front; //开机先运行空闲线程
    //os_tcb_rdy = os_tcb_cur;
