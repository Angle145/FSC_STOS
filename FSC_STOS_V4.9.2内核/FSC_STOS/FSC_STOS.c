...
/*--------------------------------------------------------------------------------*/
/*---------------------------------自定义系统指令-----------------------------------*/
//如任务超过5个，请自行增加指令,并到Cmd_Process()任务管理器任务中增加相应的代码(在本页约325行)。
...
char cmd_task_timeslice_02[]={"cmd/Task2/timeslice=%d//"}; //
char cmd_task_timeslice_03[]={"cmd/Task3/timeslice=%d//"}; //
char cmd_task_timeslice_04[]={"cmd/Task4/timeslice=%d//"}; //
char cmd_task_timeslice_05[]={"cmd/Task5/timeslice=%d//"}; //
/*-------------------------------------------*/
/*----------------设置系统时间----------------*/
char cmd_systime_set[]={"cmd/setsystime=%d/%d/%d/%d.%d.%d//"};//年/月/日/时.分.秒
/*------------------------------------------*/
#endif
/*------------------系统有关-----------------*/
char cmd_osmanager[]={"cmd/osmanage//"};         //查看系统状态
char cmd_runmode_order[]={"cmd/runmode/order//"};//时间片运行模式
char cmd_runmode_prio[]={"cmd/runmode/prio//"};  //优先级运行模式
char cmd_runmode_order_prio[]={"cmd/runmode/order+prio//"};//时间片+优先级运行模式
char cmd_oson[]={"cmd/oson//"};                  //系统关闭
char cmd_osoff[]={"cmd/osoff//"};                //系统打开
char cmd_hardreset[]={"cmd/hardreset//"};        //硬件重启
/*------------------------------------------*/
/*--------------设置系统关闭时间--------------*/
char cmd_osoff_delaytime_set[]={"cmd/setosofftime=%d/%d/%d/%d.%d.%d//"};//年/月/日/时.分.秒
/*------------------------------------------*/
/*-----------------自定义指令区--------------*/
...
/*1.系统时钟节拍*/
/*默认使用SysTick定时器   
*bit0-定时器使能 bit1-中断使能 bit2-时钟源选择(=1系统主频，=0系统主频/8)  bit16-计数溢出位*/
...
void FSC_FragMerge(void)//内存碎片整理
{
  
}
void split(struct memblock *fitting_slot,size_t size)//内存分割(从大区块分割出一部分作为申请到的内存，实质是把空闲信息结构体往后移动,再将前面的信息结构体的标志改为使用)
{
    struct memblock *new=(void*)((u8*)fitting_slot+size+sizeof(struct memblock));  //重新定位剩余空闲的地址
    new->size=(fitting_slot->size)-size-sizeof(struct memblock);                   //重新定义剩余空闲size大小
    new->free=1;                                                        //设置空闲
    new->last=fitting_slot;
   	new->next=fitting_slot->next;                                       //插入到fitting_slot与fitting_slot->next中间
    fitting_slot->size=size;
    fitting_slot->free=0;
    fitting_slot->next=new;                                             //插入到fitting_slot与fitting_slot->next中间
}
...
OS_SYSTEM OS_System;
OS_TASKPRIOGROUP TaskPrioGroup[OS_MAX_TASKS];
...
#define OSDisp_systimeset()	     OSprintf("\r\n SetSysTime OK!\r\n\r\n");
#else 
...
#define OSDisp_runmode_order_prio()  OSprintf("\r\n -----$$ OS RUNNING MODE : ORDER+PRIO  $$-----\r\n\r\n");
#define OSDisp_oson()                OSprintf("\r\n -----$$ OS AWAKEN  $$-----\r\n\r\n");
#define OSDisp_osoff()               OSprintf("\r\n -----$$ OS SLEEP  $$-----\r\n\r\n");
#define OSDisp_hardreset()           OSprintf("\r\n -----$$ HardReset $$-----\r\n\r\n");
#define OSDisp_osoff_delaytime()     OSprintf("\r\n OS ShutDown Time: %d年%d月%d日%d时%d分%d秒\r\n\r\n",\
...
#endif
			/*******************************************************************************************/	
			if(CompareCmd(cmd_osmanager)==0)     { OS_Information_Process(); }
  ...
  OSprintf("  运行模式 ：%s                     运行时间：%d年%d月%d日 %d时%d分%d秒 %dms\r\n",
	...
	OSprintf("  时钟节拍 ：%dms                            系统关闭倒计时：%d年%d月%d日 %d时%d分%d秒\r\n",\
                                             ...
                                             0,\
																						 ...
																						 0); 
#endif	
	OSprintf("  CPU占用率：%04.1f%%                          CPU最大占用率：%04.1f%% \r\r\n",100-(OSTCBTbl[0].TaskCPUOccRateCounter*100.0/OS_System.TaskTimeSliceCounter),OS_System.TaskCPUOccRateMax/10.0);
	OSprintf("  内存容量 ：%dByte\r\r\n",OS_MEMORYPOOL_SIZE);      
	OSprintf("  已用内存 ：%dByte                       剩余内存：%dByte \r\r\n",OS_MEMORYPOOL_SIZE-FSC_MemoryFreeSizeGet(),FSC_MemoryFreeSizeGet());
	OSprintf("-----------------------------------------------------------------------------------\r\n");
	...
	for(TaskCount = 0; TaskCount < OS_MAX_TASKS; TaskCount++) 
	{
		...
		{	
			OSprintf(" %04.1f%%    %03d       %-3d     %-3d     %04.1f%%     %04d     %05d     %d:%d    %s \r\n",\
									OSTCBTbl[TaskCount].TaskCPUOccRateCounter*100.0/OS_System.TaskTimeSliceCounter,\
			            OSTCBTbl[TaskCount].TaskCPUOccRateCounter*1000/OS_System.ClockCounter,\
...
{
	 INT16U TaskCount;	
	 OSTCBCur->TaskCPUOccRateCounter++;
	 OS_System.TaskTimeSliceCounter++;
	 if(OS_System.ClockCounter==0) OS_System.TaskTimeSliceCounter=0;
	 if(OS_System.TaskTimeSliceCounter==0)
	 {
		 for(TaskCount = 0; TaskCount < OS_MAX_TASKS; TaskCount++)
		 {
			 if(OSTCBTbl[OS_System.TaskNext].StkPtr != (STK32U*)0)
			 {
				 OSTCBTbl[TaskCount].TaskCPUOccRateCounter=0;
			 }
		 }
		 OS_System.TaskTimeSliceCounter=1;//从1开始计数(0不能作除数)		 
	 }  
}
...
{ 
    ...
    for(i = 0; i < OS_MAX_TASKS; i++) { 
        OSTCBTbl[i].StkPtr = (STK32U*)0; 
			  ...
			  OSTCBTbl[i].TaskCPUOccRateCounter=0; 
    ...
    OS_System.RunDriverCode=OS_FALSE;		
		...
		OS_System.TaskContinuRunTimeCounter=0;
		OS_System.RunTime_ms=0;
		OS_System.ClockCounter=0;
		OS_System.ClockOverCounter=0;
		OS_System.TimeSliceCounter=0;
...
void OSTaskCreateStatic(char* taskstr,void (*task),STK32U *stkptr,INT32U stksize,INT32U tasktimeslice,INT16U taskprio,INT8U taskstate)//静态创建任务函数(创建存放内存区域(堆栈)等)
{
    INT16U i;
#if (OS_CMD_ALL_ENABLE== 1)		
	  INT16U j; 
...
#if (OS_CMD_ALL_ENABLE == 1)	
    INT16U CupOccRate;	
		if(OS_System.TaskTimeSliceCounter>1000)
		{
			CupOccRate=(INT16U)(OSTCBTbl[0].TaskCPUOccRateCounter*1000/OS_System.TaskTimeSliceCounter);
...
{	
	u32 i;
	OSprintf("-@FSC_STOS_V4.9.2 Inside\r\n");
  ...
  while(1) 
	{
		...
		if(OS_System.Running==OS_TRUE) OS_delayMs(1000);	
...
{
   OS_System.TaskTimeSliceCounter=time;
}
void OSClockValSet(INT32U time)    //设置系统节拍计数值
{
  OS_System.ClockCounter=time;
}
#endif
/*-------------------------------------------------------------------------------------------------------------*/
/*------------------------------------------系统获取与跳转有关函数---------------------------------------------*/
#if (OS_GET_SET_SW_ENABLE == 1)
INT8U OSLockStateGet(void) //系统中是否有任务处于锁定状态，返回OS_TRUE是，OS_FALSE否
{
...
{
    return OS_System.TaskTimeSliceCounter;
}
INT32U OSClockValGet(void)    //获取系统节拍计数值
{
  return OS_System.ClockCounter;
}
...
INT8U OSTimerStateGet(INT16U TNum) //返回0 ：定时未完成，  1：完成  2:超时(超过1个时钟节拍)
{
...
void OSMutexUnlockPost(INT16U MNum) //发送互斥量 （主要用于解除mutex锁定状态,解除后所有mutex等待任务将重新竞争mutex权限）
{
	OSSchedLock();
...
INT8U OSMutexLockPend(INT16U MNum,INT32U timeout_ms) //等待互斥量 带超时时间  ，返回OS_TRUE：等待未超时  OS_FALSE：等待超时
{
	OSSchedLock();
	...
	OSTCBCur->TaskFlagBit|=_BIT_Mutex;//置位mutex标志
	OSSchedUnlock();
	OS_delayMs(timeout_ms);
	OSTCBCur->TaskFlagBit&=~_BIT_Mutex;//清除mutex标志
	if(OSTCBCur->TaskFlagBit&_BIT_TimeOut) //超时检测
	{	
		OSTCBCur->TaskFlagBit&=~_BIT_TimeOut;//清除超时标志位
		return OS_FALSE;
...
{
	OS_System.TimeSliceCounter=0; //时间切片计数器清0，准备重新计数
}
/*--------------------------------------任务切换函数--------------------------------*/
...
INT16U OSHighestPrioTaskNumGet(void)//获取已创建的所有状态任务中的最高优先级任务
{
  ...
  INT16U TaskCount,Count;	
	...
	return Count;
#elif (OS_HighSpeed_MODE == 1)	
	return TaskPrioGroup[OS_System.TaskAllStateHighestPrio].TaskTable[0];
#endif	
}
...
#elif (OS_HighSpeed_MODE == 1)	
	return TaskPrioGroup[OS_System.TaskRunStateHighestPrio].TaskTable[0];
#endif	
}
INT16U OSRdyHighestPrioTaskNumGet(void)//获取准备运行就绪最高优先级任务(返回先创建的最高优先级任务(如有同级存在))
{
  ...
  INT16U TaskCount,HighestPrioTask;	
	...
	return HighestPrioTask;
#elif (OS_HighSpeed_MODE == 1)	
	return TaskPrioGroup[OS_System.TaskRdyHighestPrio].TaskTable[0];
#endif	
}
...
INT16U OSNextRdyHighestPrioTaskNumGet(void)//获取运行就绪最高优先级任务(返回未运行过的最高优先级任务(该优先级一个轮询周期内)))
{
  ...
  INT16U TaskCount,HighestPrioTask,LastPrioTaskCount=0,SecdyHigestPrioTask,SecdyCheckFlag=0,MainCheckFlag=0;	
	...
	return HighestPrioTask;
#elif (OS_HighSpeed_MODE==1)
  return TaskPrioGroup[OS_System.TaskRdyHighestPrio].NextRunTaskNum;
#endif
}
...
#endif		 
		 if(( OSTCBTbl[TaskCount].TaskDelayMs >1 )&&((OSTCBTbl[TaskCount].TaskFlagBit&_BIT_TimeLess)==0)) 
		 {
			 OSTCBTbl[TaskCount].TaskDelayMs--;
			 if(OSTCBTbl[TaskCount].TaskDelayMs==1) OSTCBTbl[TaskCount].TaskFlagBit|=_BIT_TimeOut;//标记延时完成
		 }
	 }	
}
void OS_SysTick_Handler(void) //任务切换核心函数
{
	 ...
	 {
		 OS_System.ClockCounter++;       //系统时钟节拍计数器累加
		 if(OS_System.ClockCounter==0) OS_System.ClockOverCounter++;
     OSTaskTimeDelayCount_Process();//任务延时计数程序
		 
#if (OS_TIMECOUNTSYS_ENABLE == 1)		 
		 OSRunTimeCount_Process();   //系统运行时间统计程序
#endif			 			 
		 OS_System.TimeSliceCounter++;   //时间切片计数器(以时钟节拍为基准)
		 if(OS_System.TimeSliceCounter>=OSTCBCur->TaskTimeSlice)//时间切片溢出判断(决定一个时间切片等于多少个时钟节拍)
		 {
			 OS_System.TimeSliceCounter=0; //时间切片计数器清0，准备重新计数
